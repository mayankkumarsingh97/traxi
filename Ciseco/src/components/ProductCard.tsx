...
const ProductCard: FC<ProductCardProps> = ({
  ...
  const { name, price, description, sizes, variants, variantType, status, image_url } = data;
  ...
  const notifyAddTocart = ({ size }: { size?: string }) => {
    toast.custom(
      ...
      { position: "top-left", id: "nc-product-notify", duration: 6000 }
    );
  };
  ...
  const renderGroupButtons = () => {
    return (
      <div className="absolute bottom-1 group-hover:bottom-1 inset-x-1 flex justify-center opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all">
        <ButtonPrimary
          className="shadow-lg"
          fontSize="text-xs"
          sizeClass="py-2 px-4"
